...
//#7. Route
app.get('/weather', (req, resp) => {  //point to action='weather' in served file index.html
    const cityName = req.query.cityName; // retrieve input from name = cityName
    const params = { //specify the parameters u need from API
        q: cityName,
        units: 'metric',
        appid: keys.weather //keys.json file array[weather]
    };
    request.get('https://api.openweathermap.org/data/2.5/weather', //APL get URL
        { qs: params },
        (err, _, body) => {
            if (err) {
                resp.status(400); 
           ...
           //console.log(JSON.parse(body));
            ...
            resp.format({
                'text/html': () => {
                    resp.type('text/html');
                    resp.render('weather', {  //look up to weather.hbs in views directory
                        layout: false,
